...
public class TileEntityCaster extends TileEntity implements ITickable, IInventory
{
	...
	public static boolean isBurning(TileEntityCaster te) 
	{
	...
	{	
		...
		{
			--this.burnTime;
			BlockCaster.SetState(true, world, pos);
		}
		...
		else
		{
			...
			{
				ItemStack output = BlockCasterRecipes.getInstance().getCastingResult(inputs[0], inputs[1]);
				if(!output.isEmpty())
				{
					smelting = output;
					cookTime++;
	...
	private boolean canSmelt() 
	{
		...
		{
			ItemStack result = BlockCasterRecipes.getInstance().getCastingResult((ItemStack)this.handler.getStackInSlot(0), (ItemStack)this.handler.getStackInSlot(1));	
